...
export default function ChatDetailScreen() {
  ...
  const [workerName, setWorkerName] = useState<string>((partnerName as string) || 'Service Provider');
  const [workerPhoto, setWorkerPhoto] = useState<string | null>(null);
  const [serviceCategory, setServiceCategory] = useState<string>('Service Provider');
  ...
  const getProfilePhotoUri = (photo?: string | null, nameStr: string = 'User') => {
    if (!photo || photo.includes('default-avatar.png') || photo.includes('worker_ramesh.png')) {
      return `https://ui-avatars.com/api/?name=${encodeURIComponent(nameStr || 'User')}&background=0F2C59&color=fff&size=128`;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(data)) {
          ...
          if (data.workerPhoto) setWorkerPhoto(data.workerPhoto);
  ...
  const handleDeleteMessage = async (msgOrId?: any) => {
    if (!msgOrId) return;
    const msgId = typeof msgOrId === 'string' ? msgOrId : (msgOrId._id || msgOrId.id);
    if (!msgId) return;
  ...
  return (
    <SafeAreaView style={styles.safeContainer}>
      ...
      <View style={[styles.header, { paddingTop }]}>
        ...
        <Image 
          source={{ uri: getProfilePhotoUri(workerPhoto || undefined, workerName) }} 
          style={styles.headerAvatar} 
        />
        
        <View style={styles.headerTitleContainer}>
          <Text style={styles.headerTitle} numberOfLines={1}>{workerName}</Text>
          ...
          <View style={styles.lockedCenterContainer}>
            ...
            <Text style={styles.lockedCenterSubtitle}>
              {bookingStatus === 'Pending' 
                ? 'Chat will unlock once the worker accepts your booking request.'
                : `You need to purchase a subscription to chat with ${workerName}.`}
            </Text>
            <TouchableOpacity 
            ...
            >
              <Text style={styles.lockedReturnBtnText}>Return to Booking</Text>
            </TouchableOpacity>
          </View>
        ) : (
          <FlatList
